...
- **Serves video too**, not just images. The daemon serves any file; MIME is guessed from the extension (`.mp4` → `video/mp4`, `.mov`, `.webm`). Requests support HTTP **Range** (`206 Partial Content` + `Accept-Ranges: bytes`), which is what lets a browser stream and seek the file — mobile Safari in particular won't play an `.mp4` without it. The returned URL is just a normal link: open it in any browser on the tailnet and it plays. Keep clips under the 50 MiB per-file limit (a few seconds at simulator resolution); for longer motion, convert to a GIF instead.
